...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initLoader() {
        ...
        const duration = 2500; // 2.5 seconds total for faster loading
        ...
        const loadingInterval = setInterval(() => {
            ...
            // Make AI loader more aggressive as progress increases
            if (aiLoader) {
                const rotationSpeed = 6 - (currentProgress / 100) * 4; // 6s to 2s
                aiLoader.style.animationDuration = `${rotationSpeed}s`;
                
                // Increase glow intensity
                const loaderGlow = 20 + (currentProgress * 0.8);
                aiLoader.style.filter = `drop-shadow(0 0 ${loaderGlow}px rgba(0, 102, 255, 0.6))`;
            }
            
            // Make AI nodes more aggressive
            aiNodes.forEach((node, index) => {
                const pulseSpeed = 2 - (currentProgress / 100) * 1; // 2s to 1s
                node.style.animationDuration = `${pulseSpeed}s`;
                
                // Increase glow intensity
                const nodeGlow = 25 + (currentProgress * 0.5);
                node.style.boxShadow = `0 0 ${nodeGlow}px var(--accent-primary)`;
                
                // Increase brightness
                const brightness = 0.7 + (currentProgress / 100) * 0.6; // 0.7 to 1.3
